...
| **v2.0.0** | `待同步` | **重大更新：《象法》数据书落地 + 新栏目 + 情缘接口 + 用神土专区**：<br>① **独立数据书**：新增 `src/data/xiangfaData.ts`（《象法与十神等判断案例标准参考》入库，五行象意/富贵财官/异性婚姻/十神学业，**优先级 2**，仅服务新栏目与情缘接口，不改动 `bazidata` / `baziAnalyzer` 底层运算）<br>② **新栏目模块**：新增「九、象意」四标签页（象意/财富/感情/学历，先取既有模块数据 → 以《象法》数据书查询论断 → 命中则展示，未命中显示「目前数据不明确，仅供参考」；含「象法依据」可展开；内部说明文字不显示在页面）<br>③ **大运流年·情缘接口**：取消「Top 作用」列，改为「情缘」列——粉色 ♥ 徽标=恋爱可能、红色 喜 徽标=结婚可能（悬停显示理由），大运行、下辖十年流年、近年流年卡片同步显示，表头附图例<br>④ **用神忌神判断·土专区**：新增「土（中宫·承载制衡）」独立区，判定并解释是否取用土（按月令+阴阳占比+四土干支，含四土宜用/忌细项）<br>⑤ 版本号 1.2.1 → 2.0.0（重大更新标识） |
