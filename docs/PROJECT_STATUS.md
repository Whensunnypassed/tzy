...
**当前版本**：**v2.4.3 · 正式版**（`package.json` 与 `BaZiAnalyzerPage.tsx` 中 `APP_VERSION` 两处同步维护）
**线上地址**：https://whensunnypassed.github.io/tzy/ （由 gh-pages 分支驱动部署）
**工作区状态**：已提交 v2.4.3（命主速览·格局综合分重新设计：原局先天 + 一生大运均分加权合成）。
...
| **v2.4.1** | （本提交） | **UX 微补丁：完整生辰一条栏输入 + 移除十二时辰单独快选**（仅改 UI 与输入流程，不改动 `bazidata`/`baziCalculator`/`baziAnalyzer` 三大核心文件，小版本补丁号）：<br>① **新增 00 号 · 完整生辰一条栏**：放在四步卡片最前面，新增 `fullBirthInput` 文本框 + 「解析并填入」按钮（Enter 键等效），支持客户直接粘贴 `199910012000`（12 位：YYYYMMDDHHmm）、`1999100120`（10 位：缺分补 0）、`19991001`（8 位：缺时分补 00:00），或带分隔符的 `1999-10-01 20:00` / `1999/10/01`；内置 `parseFullBirthStr` 正则去分隔符 + 全数值合法校验（年1800-2100、月1-12、日1-31、时0-23、分0-59），失败 `toast.error` 中文原因，成功 `toast.success` 展示解析结果，然后自动 setYear/setMonth/setDay/setHour/setMinute 填入下方四步字段<br>② **移除十二时辰单独快选模块**：删除④步出生时分卡片中的 12 地支时辰 Chip 整段（含 `SHICHEN` 常量及下方 ChipButton 区），保留 0-23 小时下拉 + 0-59 分钟下拉，统一按小时精确选择（符合用户「都是按小时」需求）；常量 `SHICHEN` 替换为 `parseFullBirthStr` 解析函数<br>③ 版本号 2.4.0 → 2.4.1（补丁号，非中等级更新）；`package.json` / `APP_VERSION` / `PROJECT_STATUS.md` 同步更新 |
| **v2.4.2** | （本提交） | **新增「手动四柱」自选排盘模式**（仅改 UI 与页面层构造逻辑，不改动 `bazidata`/`baziCalculator`/`baziAnalyzer` 三大核心文件，小版本补丁号）：<br>① **排盘模式切换开关**：出生信息卡顶部新增分段开关「按日期排盘 / 手动四柱」，切换即整卡切换；手动四柱模式下，原日期排盘（完整生辰一条栏、四步卡片、一键排盘按钮）隐藏，仅显示自选四柱面板与「手动四柱排盘」按钮<br>② **自选四柱面板**：年柱/月柱/日柱/时柱四张卡片，每柱各一天干下拉（10 天干）+ 一地支下拉（12 地支），下方实时预览合并干支说明；顶部右侧实时显示当前已选四柱（如 `甲子 丙寅 甲子 甲子`）<br>③ **参照出生年份 + 性别**：因四柱六十四甲循环无法唯一反推真实出生年份，新增「参照出生年份」输入（1800-2100，仅用于定大运流年的年龄/年份）；乾/坤造性别沿用大按钮（决定大运顺逆排方向）<br>④ **页面层构造完整 BaZiChart**：`handleManualAnalyze` 用导出的 `STEM_ELEMENTS/BRANCH_ELEMENTS/STEM_YINYANG/BRANCH_YINYANG/HIDDEN_STEMS/getShiShen/MONTH_BRANCHES` 构造四柱 `Pillar`，月支索引 `MONTH_BRANCHES.indexOf(月支)`（0=寅，与 `MONTH_QI_INFO` 对齐），本地重建 60 甲子 + 按年干阴阳性别顺逆排 8 步大运（起运年龄按一岁近似，保证大运/流年年份可用），`birthInfo.solarDate` 标为「手动四柱」<br>⑤ 构建通过（`vite build` 增 6KB JS）；版本号 2.4.1 → 2.4.2；`package.json` / `APP_VERSION` / `PROJECT_STATUS.md` 同步更新 |
| **v2.4.3** | （本提交） | **命主速览·格局综合分重新设计**（仅改 UI 层展示口径，不改动 `bazidata`/`baziCalculator`/`baziAnalyzer` 三大核心文件，小版本补丁号）：<br>① **口径重构**：原卡片标题「格局综合分」却用 `wealthNobility.overallScore`、明细却拼 `mingPanScore.detail`，两个体系错位；现统一改为「**原局先天分 + 一生大运均分梯度 加权合成**」<br>② **合成公式**（UI 层用已有数据计算，不碰引擎）：`combined = 原局先天×0.6 + 一生大运均分×0.4`，其中原局先天取 `scoreMingPan().displayScore`（压缩分），一生大运均分取 `analyzeDaYunLiuNian().daYunWithFortune[].displayScore` 的八步均值；再经 `overall = clamp(50 + combined×5, 0, 100)` 映射 0-100（中枢 50，每 +1 压缩分 ≈ +5 分），字母等级沿用金字塔 9 档对齐（S+≥96 约0.1%）<br>③ **明细展示**：卡片下方改为三行——「原局先天 +X（字母）」·「一生大运均分 +Y（N 步）」·「综合 = 原局60% + 大运40%」，替换原先错位的 `mingPanScore.detail` 三项；仍沿用现有的 0-100 分数 + /100 + 进度条 + 字母徽标 + 分档标签配色体系<br>④ 版本号 2.4.2 → 2.4.3；`package.json` / `APP_VERSION` / `PROJECT_STATUS.md` 同步更新 |
