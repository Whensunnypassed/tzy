...
| 月气分析/用神忌神/命局模式等推理逻辑 | demo-mock | 在 `src/utils/tianzhiyiAnalyzer.ts` 中按天之易手册规则程序化实现：月气判断→阴阳状态→用忌标记→日干支动应→模式识别→富贵贫贱→六亲健康→大运流年全流程分析引擎 | 分析结果由引擎实时计算生成，非硬编码 mock 文本 |
